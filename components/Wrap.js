...
const Wrap = ({ Wraps, Search, data, dispatch, item, qty, setQuantities}) => {
    ...
    const handleAddtoCart = async (wrap) => {
        const finalPrice = qty * parseInt(wrap.rate);
    ...
    return (
        <>
            ...
            {Wraps.filter((e) => e.name.toLowerCase().includes(Search.toLocaleLowerCase())).length === 0 && <div>No Wraps Found</div>}
        </>
    )
}
