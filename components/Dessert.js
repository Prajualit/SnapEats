...
const Dessert = ({ Desserts, Search, qty, setQuantities, item, dispatch, data }) => {
    ...
    const handleAddtoCart = async (dessert) => {
        const finalPrice = parseInt(qty) * parseInt(dessert.rate);
    ...
    return (
        <>
            ...
            {Desserts.filter((e) => e.name.toLowerCase().includes(Search.toLocaleLowerCase())).length === 0 && <div>No Desserts Found</div>}
        </>
    )
}
